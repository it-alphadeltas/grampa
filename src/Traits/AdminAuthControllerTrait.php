...
trait AdminAuthControllerTrait
{
    ...
    {
        return view('grampa::install_admin', ['shopDomain' => request()->query('shop')]);
    }
    ...
    protected function authenticationWithCode()
    {
        ...
        if (!$api->verifyRequest(request()->all())) {
            ...
            return redirect()->route('login-admin')->with('error', 'Invalid signature');
        }
